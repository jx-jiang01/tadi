...
class Trainer(nn.Module):
    ...
    def __init__(self, w2v, cate_embeddings, subcate_embeddings, en_embeddings, 
        ...
        self.uinput_embedding = InputEmbedding(w2v, cate_embeddings, subcate_embeddings, en_embeddings)
        self.ninput_embedding = InputEmbedding(w2v, cate_embeddings, subcate_embeddings, en_embeddings)
    ...
    def forward(self, clk_ti, clk_cate, clk_subcate, clk_ti_en, ti, cate, subcate, ti_en):
        ...
        # _uemb = torch.cat([uemb, ucate_part, usubcate_part], dim=-2)
        # _iemb = torch.cat([iemb, icate_part, isubcate_part], dim=-2)
        # _uemb = _uemb.unsqueeze(1)
        # score = torch.matmul(_uemb, torch.transpose(_iemb, -1, -2))
        # pred, _ = torch.max(score, dim=-1)
        # pred = torch.sum(pred, dim=-1, keepdim=True)

        pred = torch.matmul(iemb, torch.transpose(uemb, -1, -2))
        ...
        # subcate_part = torch.cat([usubcate_part, isubcate_part], dim=-1)
        # cate_part = torch.cat([ucate_part, icate_part], dim=-1)
        # uemb_iemb_cat = torch.cat([cate_part, subcate_part], dim=-1)
        # pred2 = self.uemb_iemb_cat_fc(uemb_iemb_cat)
        
        return pred, pred1, pred2  # pred1, pred2
...
class InputEmbedding(nn.Module):
    def __init__(self, w2v, cate_embeddings, subcate_embeddings, en_embeddings) -> None:
        super(InputEmbedding, self).__init__()
        # self.cate_embedding = nn.Embedding(len(cate_embeddings), 300)
        # self.subcate_embedding = nn.Embedding(len(subcate_embeddings), 300)
...
class UserEncoder(nn.Module):
    def __init__(self, click_num, ti_max_len, en_max_len) -> None:
        ...
        self.cate_ti_attr = Attention(word_dim, word_dim, word_dim, dim)
        self.subcate_ti_attr = Attention(word_dim, word_dim, word_dim, dim)
        ...
        self.uemb_fc = nn.Sequential(
            nn.Linear(dim * 2 * 2 + en_dim + word_dim * 3, 512), # dim * 2 * 2 + 
            nn.GELU(),
            nn.Dropout(dropout),
...
class NewsEncoder(nn.Module):
    def __init__(self, ti_max_len, en_max_len) -> None:
        ...
        self.iemb_fc = nn.Sequential(
            nn.Linear(dim * 2 * 2 + en_dim + 3 * word_dim, 256), # dim * 2 * 2 + 
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(256, final_dim)
        )

        self.agg_ti_part = AggreAttention(word_dim, 512)
        self.agg_ti_en_part = AggreAttention(en_dim, 512)
...
class Attention(nn.Module):
    def __init__(self, q_input_dim, k_input_dim, v_input_dim, output_dim=256) -> None:
        ...
        self.K = nn.Sequential(
            nn.Linear(k_input_dim, 256),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(256, output_dim)
        )
        self.V = nn.Sequential(
            nn.Linear(v_input_dim, 256),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(256, output_dim)
        )
...
class Sequence(nn.Module):
    def __init__(self, input_dim, output_dim, batch_norm, layer_num=2) -> None:
        ...
        self.att = nn.TransformerEncoder(encoder_layer, num_layers=layer_num)
