...
class MindTrainDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        return torch.tensor(self.user_idx[index]), \
            ...
            torch.tensor(self.clk_ti[index]), \
...
class MindUserDataset(Dataset):
    ...
    def __getitem__(self, index):
        return self.sess_idx[index], \
            ...
            torch.tensor(np.array(self.clk_cate[index])), \
            torch.tensor(np.array(self.clk_subcate[index])), \
            torch.tensor(np.array(self.clk_ti_en[index]))
...
class MindNewsDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return self.news_idx[index], \
            ...
            torch.tensor(np.array([self.cate[index]])), \
            torch.tensor(np.array([self.subcate[index]])), \
            torch.tensor(np.array([self.ti_en[index]]))
