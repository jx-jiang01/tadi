...
from utils import cate_small_reset_dict as cate_reset_dict
from utils import subcate_small_reset_dict as subcate_reset_dict
import torch
...
def cate_embedding(vocab_embs, vocab_idx, data_dict):
    ...
    for key in data_dict:
        ...
        val = np.array(val)
        res_dict[key] = np.max(val, axis=0)
        # res_dict[key] = np.mean(val, axis=0)
    return res_dict


def preprocess_cate_subcate(path, cate_path, subcate_path):
    vocab_embs, vocab_idx = load_vocab_embs(path)
...
def load_cate_embedding(path):
    cate_id_idx, cate_emb = {"[PAD]": 0}, [np.zeros(300)]
    with open(path) as f:
        line = f.readline()
        while line:
...
def load_entity_embedding(paths):
    entity_id_idx, entity_embeddings = {"[PAD]": 0}, [np.zeros(100)]
    for path in paths:
        with open(path, encoding="utf-8") as f:
            line = f.readline()
            while line:
                terms = line.strip().split("\t")
                key = terms[0]
                val = np.array([np.float32(v) for v in terms[1:]])
...
def load_mind_news_data(paths, cate_id_idx, subcate_id_idx, en_id_idx,
...
def load_vocab_embs(w2v_path):
    vocab_embs, vocab_idx = [], {}
...
def filter_vocab_embedding(w2v_path, filtered_paths):
    word_embs = []
    ...
    with open(w2v_path, encoding="utf-8") as f:
        ...
        while line:
            ...
            if key in word_set and key not in word_idx:
                val = np.array([float(v) for v in terms[1:]], dtype=np.float32)
...
def processed_data(behavior_path, K, click_num, ti_max_len, en_max_len,
    ...
    for _clicks in clicks:
        ...
        if diff > 0:
            ...
            _click_idx = diff * [news_id_idx["[PAD]"]] + _click_idx
...
if __name__ == "__main__":
    data_root = "/data/nlp/jiangjx/data/news/mind-large/"
    ...
    for path in news_paths:
        with open(path, encoding="utf-8") as f:
            ...
            while line:
                terms = line.strip().split("\t")
                news_id, category, subcategory, title = terms[: 4]
                cate_set.add(subcategory)
                line = f.readline()
    diff_news_set = set()
    notexist = set()
    with open(data_root + "test/news.tsv", encoding="utf-8") as f:
        line = f.readline()
        while line:
            terms = line.strip().split("\t")
            news_id, category, subcategory, title = terms[: 4]
            if subcategory not in subcate_reset_dict:
                notexist.add(subcategory)
            if subcategory not in cate_set:
                diff_news_set.add(subcategory)
            line = f.readline()
    print(len(diff_news_set))
    print(diff_news_set)
    print(notexist)
